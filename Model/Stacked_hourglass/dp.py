...
class Dataset(torch.utils.data.Dataset):
    ...
    def loadImage(self, idx):
        ...
        cropped = img.crop(orig_img, c, s, (self.input_res, self.input_res))
        for i in range(np.shape(orig_keypoints)[1]):
            if orig_keypoints[0,i,0] > 0:
                orig_keypoints[0,i,:2] = img.transform(orig_keypoints[0,i,:2], c, s, (self.input_res, self.input_res))
        ...
        mat_mask = img.get_transform(center, scale, (self.output_res, self.output_res), aug_rot)[:2]

        mat = img.get_transform(center, scale, (self.input_res, self.input_res), aug_rot)[:2]
